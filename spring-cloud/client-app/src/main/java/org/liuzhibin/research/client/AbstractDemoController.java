...
public abstract class AbstractDemoController {
    ...
    abstract OrderRestService getOrderService();
    
    abstract DemoServiceFeignClient getDemoService();
    ...
    public String ping() {
        ...
        message = getDemoService().ping(message);
    ...
    public String benchmark() {
        ...
        // 预热
        getDemoService().ping("Warn-Up Ping");
        getDemoService().ping("Warn-Up Ping");
        getDemoService().ping("Warn-Up Ping");
        getDemoService().ping("Warn-Up Ping");
        ...
        for (int i = 0; i < count; i++)
            getDemoService().ping("Benchmarck Ping");
        end = System.nanoTime();
    ...
    public OrderDTO create() {
        ...
        int id = getOrderService().createOrder(order);
    ...
    public List<OrderDTO> find(@RequestParam(value = "status") OrderStatus status) {
        return getOrderService().findOrders(status);
    }
    ...
    public OrderDTO update(@PathVariable(name = "id") Integer id, @RequestParam(value = "status") OrderStatus status) {
        getOrderService().updateOrder(id, status);
        return getOrderService().getOrder(id);
    }
